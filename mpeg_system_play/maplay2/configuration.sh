...
case `uname -sr` in
  IRIX\ 5.2)
     COMPILER=CC
     COMPILERFLAGS='-O2 -DIRIX -DIndigo -DSYSTEM_PLAY'
     INCLUDEDIRS='-I/usr/include/CC -I/usr/local/include -I/usr/include -I/usr/include/sys'
     LIBRARIES=-laudio
...
#    COMPILERFLAGS='-O2 -DSunOS -DSunOS4_1_3 -DSPARC -DULAW'
  ...
  SunOS\ 4.1.1)
     COMPILER=g++
     COMPILERFLAGS='-O2 -DSunOS -DSunOS4_1_1 -DSYSTEM_PLAY -DSPARC -DULAW'
     INCLUDEDIRS=
     LIBRARIES=
     AUDIO_INCLUDES='#include <sun/audioio.h>' ;;
  SunOS\ 4.1.1-IP)
     COMPILER=g++
     COMPILERFLAGS='-O2 -DSunOS -DSunOS4_1_1 -DSPARC -DSYSTEM_PLAY -DULAW'
     INCLUDEDIRS=
     LIBRARIES=
     AUDIO_INCLUDES='#include <sun/audioio.h>' ;;
  SunOS\ 5.*)
...
#    COMPILER=/usr/lang/SC2.0.1/CC
  ...
  Linux*)
     COMPILER=g++
     COMPILERFLAGS='-O2 -m486 -funroll-loops -DSYSTEM_PLAY -DLINUX -DDAMN_INTEL_BYTE_ORDER'
     INCLUDEDIRS=
     LIBRARIES= 
